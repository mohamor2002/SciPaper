...
import handleModeratorSignIn from '../api/handleModeratorSignIn';
import { useDispatch } from 'react-redux';
import handleAdminSignIn from '../api/handleAdminSignIn';
...
const HomePage = () => {
    ...
    const [currentText, setCurrentText] = useState('If you are a moderator, sign in here');
    ...
    const textVariants = {
      ...
      useEffect(()=>{
        setCurrentText(isLogin?'If you are a moderator sign in from here!':'If you are an admin sign in from here')
        setCurrentTitle(isLogin?'Moderator?':'Admin?')
        setCurrentButton(!isLogin?'Admin Sign In':'Moderator Sign In'   )
    ...
    <div className={`relative font-br-hendrix h-screen w-full flex ${isLogin?'md:flex-row flex-col':'md:flex-row-reverse flex-col-reverse'}`}>
        ...
        <motion.form onSubmit={(e)=>isLogin?handleAdminSignIn(dispatch,e,email,password,setIsLoading):handleModeratorSignIn(dispatch,e,email,password,setIsLoading)} layout transition={{duration:1}} className=' flex-1 flex flex-col items-center justify-between'>
            
            <div className={` ${!isLogin?'hidden md:flex h-[15%]':'flex h-[20%]'} `}></div>
            <h1 className={`font-black text-dark-purple text-4xl md:text-6xl text-center ${!isLogin&&'mt-8 md:mt-0'}`}>{isLogin?'Login to Your Admin Account':'Login to Your Moderator Account'} </h1>
            <p className=' text-[#414141] opacity-50 text-lg md:text-xl'>{isLogin?'Login as Admin':'Login as Moderator'}</p>
